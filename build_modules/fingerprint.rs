...
impl Fingerprint {
    pub fn current(java_home: &Path, gradlew: &Path) -> Result<Self> {
        ...
        if gradlew.exists() {
            let mut file = File::open(gradlew)
                .with_context(|| format!("Failed to open gradle wrapper at {}", gradlew.display()))?;
            io::copy(&mut file, &mut hasher)
                .with_context(|| format!("Failed to read gradle wrapper at {}", gradlew.display()))?;
        } else {
            print_cargo_warning(&format!("Gradle wrapper not found at {}, using empty hash", gradlew.display()));
        }
        ...
        let jdk_release_file = if java_home.join("release").is_file() {
            fs::read_to_string(java_home.join("release"))
                .with_context(|| format!("Failed to read JDK release file at {}", java_home.join("release").display()))
                .ok()
        } else {
            None
        };
        ...
        let build_rs_mtime = if build_rs_path.exists() {
            let metadata = fs::metadata(&build_rs_path)
                .with_context(|| format!("Failed to get metadata for build.rs at {}", build_rs_path.display()))?;
