...
fn test_cache_output_consistency() {
    ...
    let direct_result =
        fulltext_to_tei_cached(&pdf_path, direct_config).expect("Failed to process PDF directly");
    ...
    let cached_result =
        fulltext_to_tei_cached(&pdf_path, cached_config).expect("Failed to read from cache");
    ...
    assert_eq!(
        direct_result, cached_result,
        "Cached output doesn't match direct output"
    );
}
