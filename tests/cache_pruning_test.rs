...
fn test_cache_pruning() {
    ...
    // Test 1: Process a few times to populate cache
    {
        ...
        for i in 0..5 {
            ...
            let result =
                grobid_rs::process_with_cache(&pdf_path, output_type, cache_config, || {
            ...
            assert!(
                cache_path.exists(),
                "Cache file should exist for iteration {}",
                i
            );
        ...
        assert!(
            file_count > 0,
            "Cache should contain files after processing"
        );
        println!(
    ...
    // Test 2: Run pruning and verify it cleaned up files
    {
        ...
        let (files_removed, bytes_removed) =
            grobid_rs::prune_cache(max_size).expect("Failed to prune cache");
        ...
        assert!(
            ...
            "Should have fewer files after pruning"
        );
        assert!(
            after_size < before_size,
            "Cache should be smaller after pruning"
        );
    ...
    // Test 3: Clear cache and verify all files are removed
    {
        ...
        // Clear cache
        let (files_removed, _) = grobid_rs::clear_cache().expect("Failed to clear cache");
