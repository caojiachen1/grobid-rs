...
fn test_thread_safety() {
    ...
    assert_eq!(final_results.len(), num_operations, "Not all threads completed");
    
    // Ensure all operations succeeded
    let failures: Vec<_> = final_results.iter()
        .filter(|r| r.contains("failed"))
        .collect();
    
    assert!(failures.is_empty(), "Some thread operations failed: {:?}", failures);
}