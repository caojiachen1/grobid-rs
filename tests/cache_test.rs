...
use grobid_rs::{init_with_config, GrobidConfig, CacheConfig, OutputType, fulltext_to_tei_cached};
...
fn test_cache_behavior() {
    ...
    // Test 1: First run should process the PDF and store in cache
    {
        ...
        let result1 = fulltext_to_tei_cached(&pdf_path, cache_config)
            .expect("Failed to process PDF");
        
        // Check that something was cached
        assert_eq!(count_cache_files(), 1, "Cache should contain 1 file after first run");
    ...
    // Test 2: Second run should use the cache
    {
        ...
        let result2 = fulltext_to_tei_cached(&pdf_path, cache_config)
            .expect("Failed to process PDF");
        
        // Check that cache file count hasn't changed
        assert_eq!(count_cache_files(), 1, "Cache should still contain 1 file after second run");
    ...
    // Test 3: Run with force_reprocess should ignore cache
    {
        ...
        assert_eq!(count_cache_files(), 1, "Cache should still contain 1 file after force reprocess");
    ...
    // Test 4: Run with cache disabled should not use or update cache
    {
        ...
        // Check that cache file count is still the same
        assert_eq!(count_cache_files(), 1, "Cache should still contain 1 file after disabled cache run");
