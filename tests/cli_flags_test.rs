...
fn test_cli_default_flags() {
    ...
    assert!(
        cache_files_count > 0,
        "No cache files were created with default flags"
    );
...
fn test_cli_force_reprocess_flag() {
    ...
    assert!(
        second_mod_time > first_mod_time,
        "Cache file wasn't updated with --force-reprocess"
    );
...
fn test_cli_no_cache_flag() {
    ...
    assert!(
        status.success(),
        "CLI execution failed with --no-cache flag"
    );
    ...
    assert_eq!(
        cache_files_count, 0,
        "Cache files were created despite --no-cache flag"
    );
...
fn test_cli_skip_existing_flag() {
    ...
    assert_eq!(
        ...
        "Cache file was updated despite --skip-existing flag"
    );
