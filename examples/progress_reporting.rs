...
fn process_document(
    ...
    send_progress(ProcessingStage::Initializing, &format!("Starting to process {}", filename))?;
    
    send_progress(ProcessingStage::PdfToXml, "Running pdfalto for XML conversion")?;
    
    send_progress(ProcessingStage::SegmentingDocument, "Applying segmentation model")?;
    
    send_progress(ProcessingStage::ProcessingHeader, "Extracting title, authors, and abstracts")?;
    
    send_progress(ProcessingStage::ProcessingBody, "Processing main document content")?;
    ...
    send_progress(ProcessingStage::ProcessingReferences, "Extracting and parsing references")?;
    
    send_progress(ProcessingStage::ProcessingFiguresTables, "Processing figures and tables")?;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let processing_thread = thread::spawn(move || {
        process_document(&pdf_path_clone, tx, should_cancel)
    });
    ...
    let result = match processing_thread.join() {
        Ok(Ok(output)) => {
            println!("\n\nSuccess! Processing completed in {:.2} seconds", start_time.elapsed().as_secs_f32());
            println!("\nOutput sample:");
            println!("{}", output);
            Ok(())
        },
