...
pub fn get_cache_summary() -> Result<String, GrobidError> {
    ...
    summary.push_str(&format!(
        "  Estimated time saved: {:.2} seconds\n",
    ...
    for entry in fs::read_dir(&cache_dir).map_err(|e| {
        ...
        if metadata.is_file() {
            ...
            let last_accessed = metadata.accessed().unwrap_or_else(|_| {
                debug!(
                    "Could not get access time for {}, falling back to modified time",
                    path.display()
                );
                metadata.modified().unwrap_or_else(|_| {
                    debug!(
                        "Could not get modified time for {}, using current time",
                        path.display()
                    );
                    SystemTime::now()
                })
            ...
            debug!(
                "Adding cache file: {} (size: {}, last accessed: {:?})",
                path.display(),
...
pub fn prune_cache(max_size_bytes: u64) -> Result<(usize, u64), GrobidError> {
    ...
    if current_size <= max_size_bytes {
        info!(
            "Cache is already under the size limit ({} <= {})",
    ...
    info!(
        "Pruning cache: current size {} exceeds limit {} (needs to remove at least {} bytes)",
        format_bytes(current_size),
        format_bytes(max_size_bytes),
    ...
    if files_removed == 0 && bytes_to_remove > 0 {
        ...
        if all_files.is_empty() {
            ...
            info!(
                "Found {} files to consider for force removal",
                all_files.len()
            );
        }
            ...
            if let Ok(metadata) = fs::metadata(&path) {
                if metadata.is_file() {
                    ...
                    match fs::remove_file(&path) {
                        ...
                        Err(e) => {
                            warn!(
                                "Failed to force-remove cache file {}: {}",
                                path.display(),
                                e
                            );
                        }
                ...
                warn!(
                    "Failed to get metadata for file to force-remove: {}",
                    path.display()
                );
            }
        }
    ...
    info!(
        "Cache pruning complete: removed {} files ({}) - new size: {}",
        files_removed,
...
pub fn start_background_gc() {
    ...
    if let Err(e) = std::thread::Builder::new()
        ...
        .spawn(move || {
            debug!(
                "Started background cache GC thread (interval: {:?})",
                interval
            );
...
pub fn check_and_prune_if_needed() -> Result<(), GrobidError> {
    ...
    if current_size > max_size {
        debug!(
            "Cache size {} exceeds limit {}, pruning",
            format_bytes(current_size),
            format_bytes(max_size)
        );
