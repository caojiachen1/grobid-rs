...
pub(crate) fn call_engine_process_method_with_file_input(
    ...
    let j_file_obj = handle.new_object(file_cls, "(Ljava/lang/String;)V", &[JValue::from(&j_path_str)])?;

    let engine = handle.engine()?;
    let j_result_string_obj = handle.call_method(
        engine,
        method_name,
    ...
    let result_string: String = handle.get_string(&JString::from(j_result_string_obj))?.into();
    Ok(result_string)
}
...
pub(crate) fn call_engine_fulltext_to_tei(
    ...
    let cfg_obj = handle.call_static_method(cfg_cls, "defaultInstance", "()Lorg/grobid/core/engines/config/GrobidAnalysisConfig;", &[])?.l()?;
    ...
    debug!("Fulltext TEI extraction successful, TEI length: {} bytes", tei_string.len());
    Ok(tei_string)
}
...
pub(crate) fn call_engine_process_header(
    ...
    let engine = handle.engine()?;
    let j_result_obj = handle.call_method(
        engine,
        "processHeader",
    ...
    debug!("Header processing successful, TEI length: {} bytes", result_string.len());
    Ok(result_string)
}
...
pub(crate) fn call_engine_process_references(
    ...
    let file_cls = handle.find_class("java/io/File")?;
    let j_path_str = handle.new_string(pdf_path.to_string_lossy())?;
    let j_file = handle.new_object(file_cls, "(Ljava/lang/String;)V", &[JValue::from(&j_path_str)])?;
    
    // Call Engine.processReferences(File, int)
    // The int parameter is the consolidation option (0 = no consolidation)
    let engine = handle.engine()?;
    let j_bib_list = handle.call_method(
        engine,
        "processReferences",
    ...
    let j_result_obj = handle.call_static_method(
...
pub fn fulltext_to_tei(pdf: &Path) -> Result<String, GrobidError> {
    ...
    with_env(|handle| {
        call_engine_fulltext_to_tei(handle, pdf)
    })
}
...
pub fn process_header(pdf: &Path) -> Result<String, GrobidError> {
    ...
    with_env(|handle| {
        call_engine_process_header(handle, pdf)
    })
}
...
pub fn process_references(pdf: &Path) -> Result<String, GrobidError> {
    ...
    with_env(|handle| {
        call_engine_process_references(handle, pdf)
    })
}