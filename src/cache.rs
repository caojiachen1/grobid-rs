...
pub fn get_cache_dir() -> Result<PathBuf, GrobidError> {
    ...
    if let Ok(env_cache_dir) = std::env::var("GROBID_RS_CACHE_DIR") {
        ...
        if !cache_dir.exists() {
            trace!("Creating cache directory from env var: {}", cache_dir.display());
    ...
    let proj_dirs = ProjectDirs::from("rs", "", "grobid-rs")
        .ok_or_else(|| GrobidError::InvalidInput("Failed to determine cache directory".to_string()))?;
...
pub fn get_cache_path(pdf_path: &Path, output_type: OutputType) -> Result<PathBuf, GrobidError> {
    ...
    trace!("Cache path for {}: {}", pdf_path.display(), cache_path.display());
    Ok(cache_path)
}
...
pub fn cache_exists(pdf_path: &Path, output_type: OutputType) -> Result<bool, GrobidError> {
    ...
    if exists {
        debug!("Cache hit for {}: {}", pdf_path.display(), cache_path.display());
    } else {
        debug!("Cache miss for {}: {}", pdf_path.display(), cache_path.display());
    }
...
pub fn read_cache(pdf_path: &Path, output_type: OutputType) -> Result<String, GrobidError> {
    ...
    let mmap = unsafe { Mmap::map(&file) }
        .map_err(|e| GrobidError::Io(std::io::Error::new(std::io::ErrorKind::Other, 
    ...
    let content = match std::str::from_utf8(&mmap) {
        Ok(s) => s.to_string(),
        Err(e) => return Err(GrobidError::InvalidInput(
            format!("Cache file contains invalid UTF-8: {}", e)
        )),
    };
    ...
    if let Ok(file_content) = fs::read(pdf_path) {
        ...
        let hash_from_path = cache_path.file_stem()
        ...
        if hash_from_path != expected_hash {
            ...
            return Err(GrobidError::InvalidInput(
                format!("Cache validation failed: hash mismatch for {}", pdf_path.display())
            ));
        }
    }
    ...
    info!("Read {} bytes from cache for {} using zero-copy", file_size, pdf_path.display());
    Ok(content)
}

/// Get the path to a cached result if it exists
pub fn get_cached_path(pdf_path: &Path, output_type: OutputType) -> Result<Option<PathBuf>, GrobidError> {
    let cache_path = get_cache_path(pdf_path, output_type)?;
...
pub fn write_cache(pdf_path: &Path, output_type: OutputType, content: &str) -> Result<(), GrobidError> {
    let cache_path = get_cache_path(pdf_path, output_type)?;
    
    debug!("Writing to cache: {}", cache_path.display());
    
    // Create a temporary file to write to atomically
    let dir = cache_path.parent()
        .ok_or_else(|| GrobidError::InvalidInput("Failed to determine cache file parent directory".to_string()))?;
    ...
    info!("Wrote {} bytes to cache for {}", content.len(), pdf_path.display());
    Ok(())
}
    ...
    F: FnOnce() -> Result<String, GrobidError>
{
    ...
    let use_cache = match (cache_exists(pdf_path, output_type)?, config.skip_existing, config.force_reprocess) {
    ...
    if use_cache {
        ...
        match read_cache(pdf_path, output_type) {
            Ok(content) => {
                ...
                CACHE_TIME_SAVED_MS.fetch_add(estimated_saved.as_millis() as usize, Ordering::Relaxed);
