...
use dotenv::dotenv;
use build_modules::{
    common::{print_cargo_warning, GROBID_RS_ASSETS_PATH_ENV_VAR, FORCE_GROBID_REBUILD_ENV_VAR, GROBID_DIR_NAME_PREFIX, GROBID_VERSION, GROBID_HOME_DIR_NAME, GROBID_JAR_NAME_PREFIX, GROBID_ONEJAR_NAME_SUFFIX, JLINK_RUNTIME_SUBDIR_NAME, JRE_SUCCESS_MARKER_FILE},
...
fn use_vendored_files(vendor_dir: &Path, deployment_dir: &Path) -> Result<()> {
    print_cargo_warning(&format!("Using vendored files from {} to {}", vendor_dir.display(), deployment_dir.display()));
    ...
    if target_home_path.exists() {
        fs::remove_dir_all(&target_home_path)
            .with_context(|| format!("Failed to remove existing grobid-home directory: {}", target_home_path.display()))?;
    }
    
    copy_dir_recursive(&vendor_home_path, &target_home_path)
        .with_context(|| format!("Failed to copy grobid-home from {} to {}", vendor_home_path.display(), target_home_path.display()))?;
    ...
    if target_runtime_dir.exists() {
        fs::remove_dir_all(&target_runtime_dir)
            .with_context(|| format!("Failed to remove existing runtime directory: {}", target_runtime_dir.display()))?;
    }
    ...
    for entry in fs::read_dir(&vendor_jre_dir)? {
        ...
        if path.is_dir() {
            ...
            copy_dir_recursive(&path, &target_platform_dir)
                .with_context(|| format!("Failed to copy JRE platform dir from {} to {}", path.display(), target_platform_dir.display()))?;
        }
    }
...
fn main() -> Result<()> {
    ...
    if force_clean {
        ...
        let grobid_version_dir = assets_dir.join(format!("{}{}", GROBID_DIR_NAME_PREFIX, GROBID_VERSION));
        if grobid_version_dir.exists() {
            fs::remove_dir_all(&grobid_version_dir)
                .with_context(|| format!("Failed to remove cached Grobid version directory: {}", grobid_version_dir.display()))?;
        }
    } else {
        ...
        let runtime_dir = assets_dir.join(format!("{}{}", GROBID_DIR_NAME_PREFIX, GROBID_VERSION))
            .join(JLINK_RUNTIME_SUBDIR_NAME);
        let jre_marker = assets_dir.join(format!("{}{}", GROBID_DIR_NAME_PREFIX, GROBID_VERSION))
    ...
    if !deployment_dir.exists() {
        fs::create_dir_all(&deployment_dir).with_context(|| {
            format!("Failed to create deployment directory: {}", deployment_dir.display())
        })?;
    }
    ...
    println!("cargo:rustc-env={}={}", GROBID_RS_ASSETS_PATH_ENV_VAR, deployment_dir.display());
    ...
    if !jlink_dir.exists() || !jre_marker.exists() || force_clean {
        ...
        if jlink_dir.exists() {
            print_cargo_warning(&format!("Removing existing JRE runtime for rebuild at {}", jlink_dir.display()));
            fs::remove_dir_all(&jlink_dir)
                .with_context(|| format!("Failed to remove existing JRE runtime directory: {}", jlink_dir.display()))?;
        }
        if jre_marker.exists() {
            fs::remove_file(&jre_marker)
                .with_context(|| format!("Failed to remove JRE success marker: {}", jre_marker.display()))?;
        }
        let jlink_result = ensure_jlink_runtime(&java_home, &deployment_dir)?;
        print_cargo_warning(&format!("JRE runtime created at {}", jlink_result.display()));
    } else {
        print_cargo_warning(&format!("Using existing JRE runtime at {}", jlink_dir.display()));
    }
